...
def compute_loss(net_output, binary_label, instance_label, loss_type = 'FocalLoss'):
    ...
    if(loss_type == 'FocalLoss'):
        loss_fn = FocalLoss(gamma=2, alpha=[0.2, 0.8])
...
def train_model(model, optimizer, scheduler, dataloaders, dataset_sizes, device, loss_type='FocalLoss', num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        if scheduler is not None:  # 在每个epoch结束后调用scheduler
    ...
    return model, training_log, time_elapsed
...
def load_model(model, checkpoint_path):
    ...
    try:
        ...
        logging.error("Checkpoint file not found. Training from scratch...")
